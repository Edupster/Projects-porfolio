...
: ''').lower()
        ...
        #if the user enters the option statiscs run the block below
        if menu == "s":
            os.system("cls")

            #Calclate the number of users in the system getting the lenght of the list "num_users"
            num_users=len(user_password)
            num_tasks=0
            f=open("tasks.txt","r")

            #Start a for loop to read each line in the file "tasks.txt" and count the number of lines which is the same than the number of tasks
            for i in f:
...
: ''').lower()
    ...
    #If the "admin" has logged to register a new user, then proceed to request the new user credentials
